...
interface VirtualizedTableProps {
  ...
  onDataUpdate?: (rowIndex: number, field: string, value: number) => void
...
export const VirtualizedTable = memo(({ 
  ...
  const handleCellBlur = () => {
    if (editingCell) {
      ...
      if (!isNaN(newValue) && onDataUpdate) {
        onDataUpdate(editingCell.id - 1, editingCell.field, newValue)
      }
      setEditingCell(null)
    }
  }
    ...
    <div className="overflow-x-auto">
      ...
      <div className="sticky top-0 z-20 bg-background border-b border-border">
        <div className="flex">
          ...
          <div className="p-1 text-left text-xs font-medium text-muted-foreground bg-background flex-shrink-0" style={{ width: `${getColumnWidth('Action')}px` }}>
            Action
          </div>
        </div>
      </div>
              ...
              return (
                ...
                >
                  ...
                  {columns.map((col) => (
                    <div
                      key={col}
                      className="p-1 text-xs text-foreground cursor-pointer hover:bg-accent/30 flex-shrink-0"
                      onClick={(e) => {
                        e.stopPropagation()
                        handleCellClick(actualIndex + 1, col, row[col])
                      }}
                      style={{ width: `${getColumnWidth(col)}px` }}
                    >
                      {editingCell?.id === actualIndex + 1 && editingCell?.field === col ? (
                        <input
                          type="number"
                          step="0.1"
                          value={editValue}
                          ...
                          autoFocus
                          className="w-full bg-background border border-primary rounded text-xs"
                          style={{ 
                            width: '100%',
                            minWidth: '0',
                            maxWidth: '100%',
                  ...
                  <div className="p-1 flex-shrink-0" style={{ width: `${getColumnWidth('Action')}px` }}>
                    <button
                      onClick={(e) => {
                        e.stopPropagation()
                        handleRowRemove(actualIndex)
                      }}
